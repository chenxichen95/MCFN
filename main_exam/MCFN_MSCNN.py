...
class Fusion_Layer(nn.Module):
    def __init__(self, args):
        ...
        self.G1 = Linear(args.gru_hidden_size*4, args.gru_hidden_size, activations=True)
        self.G2 = Linear(args.gru_hidden_size*4, args.gru_hidden_size, activations=True)
        self.G3 = Linear(args.gru_hidden_size*4, args.gru_hidden_size, activations=True)
...
class MCFN_MSCNN(nn.Module):
    def __init__(self, args, use_w2v=False, weight=None):
        super(MCFN_MSCNN, self).__init__()
    ...
    def forward(
        ...
        # multi-attention
        p_q_attn1, p_a_attn1 = self.attention1(q_encoding, p_a_encoding, q_input_mask, p_a_input_mask)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_q_attn1, n_a_attn1 = self.attention1(q_encoding, n_a_encoding, q_input_mask, n_a_input_mask)
        ...
        # calc loss
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            loss, score = self.loss(p_q_pooling_1, p_a_pooling_1, n_q_pooling_1, n_a_pooling_1)
            return loss, score
        else:
            score = self.loss(p_q_pooling_1, p_a_pooling_1)
            return score
