...
class OnlySingleCNN(nn.Module):
    def __init__(self, args, use_w2v=False, weight=None):
        super(OnlySingleCNN, self).__init__()
    ...
    def forward(
        ...
        # single-scale cnn
        q_cnn1 = self.cnn(q_embedding, q_input_mask)
        p_a_cnn1 = self.cnn(p_a_embedding, p_a_input_mask)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_cnn1 = self.cnn(n_a_embedding, n_a_input_mask)

        # max pooling
        q_pooling = self.pooling(q_cnn1, q_input_mask)
        p_a_pooling = self.pooling(p_a_cnn1, p_a_input_mask)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_pooling = self.pooling(n_a_cnn1, n_a_input_mask)
