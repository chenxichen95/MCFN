...
class Fusion_Layer(nn.Module):
    def __init__(self, args):
        ...
        self.G1 = Linear(args.cnn_kernel_num, args.gru_hidden_size, activations=True)
        self.G2 = Linear(args.cnn_kernel_num, args.gru_hidden_size, activations=True)
        self.G3 = Linear(args.cnn_kernel_num, args.gru_hidden_size, activations=True)
        self.G = Linear(args.gru_hidden_size*3, args.gru_hidden_size, activations=True)
        self.pooling = Pooling()

    def forward(self, input, mask):
        a1 = self.G1(input)
        a_pool = self.pooling(a1, mask)
        return a_pool
...
class MCFN_co(nn.Module):
    def __init__(self, args, use_w2v=False, weight=None):
        super(MCFN_co, self).__init__()
    ...
    def forward(
        ...
        # max pooling
        q_pooling_1 = self.fusion_q(q_cnn1, q_input_mask)
        q_pooling_2 = self.fusion_q(q_cnn2, q_input_mask)
        q_pooling_3 = self.fusion_q(q_cnn3, q_input_mask)
        q_pooling = torch.cat([q_pooling_1, q_pooling_2, q_pooling_3], dim=-1)

        p_a_pooling_1 = self.fusion_a(p_a_cnn1, p_a_input_mask)
        p_a_pooling_2 = self.fusion_a(p_a_cnn2, p_a_input_mask)
        p_a_pooling_3 = self.fusion_a(p_a_cnn3, p_a_input_mask)
        p_a_pooling = torch.cat([p_a_pooling_1, p_a_pooling_2, p_a_pooling_3], dim=-1)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_pooling_1 = self.fusion_a(n_a_cnn1, n_a_input_mask)
            n_a_pooling_2 = self.fusion_a(n_a_cnn2, n_a_input_mask)
            n_a_pooling_3 = self.fusion_a(n_a_cnn3, n_a_input_mask)
