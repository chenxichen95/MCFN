...
class Pooling(nn.Module):
    def forward(self, inputs, mask):
        return inputs.masked_fill_(~mask, -float('inf')).max(dim=1)[0]
...
class MCFN(nn.Module):
    def __init__(self, args, use_w2v=False, weight=None):
        ...
        if self.use_w2v:
            ...
            pass
        self.bigru = BiGRU(300, args.gru_hidden_size, bidirectional=True)
        self.cnn = MultiScaleCNN(args)
        self.attention1 = OneScaleOneAttentionLayer2(args)
        self.attention2 = OneScaleOneAttentionLayer2(args)
        self.attention3 = OneScaleOneAttentionLayer2(args)

        self.fusion_q = Fusion_Layer(args)
        self.fusion_a = Fusion_Layer(args)
    ...
    def forward(
        ...
        # bi-gru
        q_encoding = self.bigru(q_embedding, q_input_mask.sum(dim=-1))
        p_a_encoding = self.bigru(p_a_embedding, p_a_input_mask.sum(dim=-1))
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_encoding = self.bigru(n_a_embedding, n_a_input_mask.sum(dim=-1))
        ...
        # multi-scale cnn
        q_cnn1, q_cnn2, q_cnn3 = self.cnn(q_encoding, q_input_mask)
        p_a_cnn1, p_a_cnn2, p_a_cnn3 = self.cnn(p_a_encoding, p_a_input_mask)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_cnn1, n_a_cnn2, n_a_cnn3 = self.cnn(n_a_encoding, n_a_input_mask)

        # multi-attention
        p_q_attn1, p_a_attn1 = self.attention1(q_cnn1, p_a_cnn1, q_input_mask, p_a_input_mask)
        p_q_attn2, p_a_attn2 = self.attention2(q_cnn2, p_a_cnn2, q_input_mask, p_a_input_mask)
        p_q_attn3, p_a_attn3 = self.attention3(q_cnn3, p_a_cnn3, q_input_mask, p_a_input_mask)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_q_attn1, n_a_attn1 = self.attention1(q_cnn1, n_a_cnn1, q_input_mask, n_a_input_mask)
            n_q_attn2, n_a_attn2 = self.attention2(q_cnn2, n_a_cnn2, q_input_mask, n_a_input_mask)
            n_q_attn3, n_a_attn3 = self.attention3(q_cnn3, n_a_cnn3, q_input_mask, n_a_input_mask)

        # max pooling
        p_q_pooling_1 = self.fusion_q(p_q_attn1, q_input_mask)
        p_q_pooling_2 = self.fusion_q(p_q_attn2, q_input_mask)
        p_q_pooling_3 = self.fusion_q(p_q_attn3, q_input_mask)
        p_q_pooling = torch.cat([p_q_pooling_1, p_q_pooling_2, p_q_pooling_3], dim=-1)

        p_a_pooling_1 = self.fusion_a(p_a_attn1, p_a_input_mask)
        p_a_pooling_2 = self.fusion_a(p_a_attn2, p_a_input_mask)
        p_a_pooling_3 = self.fusion_a(p_a_attn3, p_a_input_mask)
        p_a_pooling = torch.cat([p_a_pooling_1, p_a_pooling_2, p_a_pooling_3], dim=-1)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_q_pooling_1 = self.fusion_q(n_q_attn1, q_input_mask)
            n_q_pooling_2 = self.fusion_q(n_q_attn2, q_input_mask)
            n_q_pooling_3 = self.fusion_q(n_q_attn3, q_input_mask)
            n_q_pooling = torch.cat([n_q_pooling_1, n_q_pooling_2, n_q_pooling_3], dim=-1)

            n_a_pooling_1 = self.fusion_a(n_a_attn1, n_a_input_mask)
            n_a_pooling_2 = self.fusion_a(n_a_attn2, n_a_input_mask)
            n_a_pooling_3 = self.fusion_a(n_a_attn3, n_a_input_mask)
            n_a_pooling = torch.cat([n_a_pooling_1, n_a_pooling_2, n_a_pooling_3], dim=-1)

        # calc loss
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            loss, score = self.loss(p_q_pooling, p_a_pooling, n_q_pooling, n_a_pooling)
            return loss, score
        else:
            score = self.loss(p_q_pooling, p_a_pooling)
            return score
