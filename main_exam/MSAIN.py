...
class MSAIN(nn.Module):
    def __init__(self, args, use_w2v=False, weight=None):
        ...
        self.attention1 = OneScaleAttentionLayer(args)
        self.attention2 = OneScaleAttentionLayer(args)
        self.attention3 = OneScaleAttentionLayer(args)
    ...
    def forward(
        ...
        # residual connect
        q_encoding = torch.cat([q_encoding, q_embedding], dim=-1)
        p_a_encoding = torch.cat([p_a_encoding, p_a_embedding], dim=-1)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_encoding = torch.cat([n_a_encoding, n_a_embedding], dim=-1)
        ...
        # max pooling
        p_q_pooling = torch.stack([p_q_attn1, p_q_attn2, p_q_attn3], dim=-1).max(dim=-1)[0]
        p_a_pooling = torch.stack([p_a_attn1, p_a_attn2, p_a_attn3], dim=-1).max(dim=-1)[0]
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_q_pooling = torch.stack([n_q_attn1, n_q_attn2, n_q_attn3], dim=-1).max(dim=-1)[0]
            n_a_pooling = torch.stack([n_a_attn1, n_a_attn2, n_a_attn3], dim=-1).max(dim=-1)[0]
