...
class SingleScaleCNN(nn.Module):
    def __init__(self, args):
        ...
        self.in_channels = args.gru_hidden_size*2
        ...
        nn.init.normal_(self.cnn_layer1.weight, std=math.sqrt(2. / (self.in_channels * 1)))
...
class OnlyBiGRU(nn.Module):
    def __init__(self, args, use_w2v=False, weight=None):
        super(OnlyBiGRU, self).__init__()
    ...
    def forward(
        ...
        # max pooling
        q_pooling = self.pooling(q_encoding, q_input_mask)
        p_a_pooling = self.pooling(p_a_encoding, p_a_input_mask)
        if isinstance(n_a_input_ids, torch.Tensor) or isinstance(n_a_input_ids_w2v, torch.Tensor):
            n_a_pooling = self.pooling(n_a_encoding, n_a_input_mask)
...
def save_model(model, global_step, args, logging, running_time, benchmark):
    log_dir_name = running_time
    save_dir = os.path.join(args.output_dir, log_dir_name, f'benchmark-{benchmark}')
    ...
    logging.info(f'save model: {save_dir}/pretrain_bert_pytorch_model_gstep{global_step}.bin')
